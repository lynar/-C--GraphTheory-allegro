...
BFS::BFS(Maze maze): Algorithm(maze)
{
    //ctor
}
...
std::vector <Noeud> BFS::perform(Graphic& g)
{
    return perform(m_maze, g);
}

//BFS
std::vector <Noeud> BFS::perform(Maze& maze, Graphic& g)
{

    std::vector<Noeud> file; // File
    ...
    file.push_back(depart); // Le premier noeud est enfilé

    while (predecesseurs.empty()) //Tant que la file n'est pas vide
    {
    maze.detecterLesDeadLock();

        Noeud actuel = file[pivot]; // le premier noeud est défilé
            ...
            for (int j=0; j<visite.size(); j++)
            {
                    ...
                    //Si on peut bouger une boite vers la direction souhaitee
                {
...

                    {
                        ...
                        file.push_back(next); //On le rajoute à la file
                    }
                }
        ...
        if (compteur==maze.getGoals().size()) // Si on l'a trouvée
        {
            ...
            do
            {
                temp = file[temp.getPredecesseur()];
                predecesseurs.push_back(temp);
            }
